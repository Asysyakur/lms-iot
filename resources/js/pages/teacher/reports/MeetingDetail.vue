...
student,
  index
              ...
              <td class="px-4 py-3 text-center align-middle">
                ...
                <!-- TYPE BOTH -->
                <div v-if="meeting.practiceType === 'both'" class="flex flex-col items-center gap-1.5">
                  <a v-if="student.practice" :href="student.practice" target="_blank"
                    class="inline-flex max-w-fit whitespace-nowrap rounded-full bg-blue-100 px-3 py-1.5 text-[11px] font-semibold leading-none text-blue-700 hover:bg-blue-200">
                    Buka Link
                  </a>
                  <div v-if="student.practiceText"
                    class="flex w-full min-w-0 items-center rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs leading-5 text-slate-700">
                    <span class="block min-w-0 truncate">{{ previewAnswer(student.practiceText) }}</span>
                  </div>
                  <span v-if="!student.practice && !student.practiceText" class="text-slate-400">
                    -
                  </span>
                </div>

                <!-- TYPE TEXT -->
                <div v-else-if="meeting.practiceType === 'text'"
                  class="flex w-full min-w-0 items-center rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs leading-5 text-slate-700">
                  <span class="block min-w-0 truncate">{{ previewAnswer(student.practiceText) }}</span>
                </div>
                ...
                <a v-else-if="student.practice" :href="student.practice" target="_blank"
                  class="inline-flex max-w-fit whitespace-nowrap rounded-full bg-blue-100 px-3 py-2 text-[11px] font-semibold leading-none text-blue-700 hover:bg-blue-200">
                  Buka Link
                </a>
        ...
        <div v-if="selectedStudent" class="max-h-[75vh] space-y-6 overflow-y-auto bg-slate-100 p-6">
          ...
          <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="p-5">
              ...
              <template v-if="meeting.practiceType === 'both'">
                <div class="space-y-3">
                  <div v-if="selectedStudent.practice">
                    <a :href="selectedStudent.practice" target="_blank"
                  ...
                  <div v-if="selectedStudent.practiceText">
                    <h4 class="mb-1 text-xs font-semibold text-slate-500">Jawaban Teks:</h4>
                    <p class="whitespace-pre-wrap break-words text-sm leading-7 text-slate-700">
                      {{ selectedStudent.practiceText }}
                    </p>
                  </div>

                  <span v-if="!selectedStudent.practice && !selectedStudent.practiceText" class="text-sm text-slate-400">
                    -
                  </span>
                </div>
              </template>

              <template v-else-if="meeting.practiceType === 'text'">
